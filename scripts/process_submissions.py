...
def process_row(row: dict, drive, gh_repo) -> tuple[bool, str, list[str]]:
    ...
    # ── Open PR ─────────────────────────────────────────────────────────────
    added = [f"`data/commentaries/{slug}.mdx`"]
    if image_path:
        added.append(
            f"`public/static/images/articles/article{art_num}{Path(image_path).suffix}`"
        )
    if "New author" in author_status:
        added.append(f"`data/people/{author_slug}.mdx`")

    missing: list[str] = []
    if not image_path:
        missing.append(
            "Commentary image (`Commentry Photo Link` was empty or invalid) — add manually before merging"
        )
    if not col(COL_SUMMARY):
        missing.append("Summary is empty — add to frontmatter before merging")
    if not col(COL_TAGS):
        missing.append("Tags are empty — add to frontmatter before merging")
    if "no photo" in author_status.lower():
        missing.append(
            "Author photo missing — add to `public/static/images/people/` before merging"
        )
    ...
    if missing:
        pr_sections += [
            "",
            "## ⚠️ Missing — Requires Attention Before Merging",
            "\n".join(f"- {m}" for m in missing),
        ]
    if notes:
        pr_sections += ["", "## Notes", "\n".join(f"- {n}" for n in notes)]

    # Placeholder body without preview URLs (need PR number first)
    pr_body_base = "\n".join(pr_sections)
    ...
    # Now we have the PR number — add preview links and verification checklist
    preview_base = (
    ...
    pr_sections += [
        "",
        "## Preview",
        f"- [Commentary]({preview_base}/commentaries/{slug})",
        f"- [Author page]({preview_base}/authors/{author_slug})",
        "",
        "## Verification checklist",
        "- [ ] Commentary body is formatted correctly",
        "- [ ] Article image loads correctly",
        "- [ ] Tags appear in the sidebar",
        f"- [ ] Live URL after merge: https://www.thestatecraft.in/commentaries/{slug}",
    ]
    pr.edit(body="\n".join(pr_sections))
